...
class ScoresLoader:
    ...
    def load(self, file_names, channel_id=-1):
        '''
        ...
        for i, name in enumerate(tqdm(file_names, desc='Loading scores')):
            ...
            if "ESA" in name_split[0]:
                assert channel_id > 0
                data_split_name = name_split[1].split(".")[0]
                paths = [os.path.join(self.scores_path, 
                                      name_split[0] + "-" + data_split_name , 
                                      detector, 
                                      'score', 
                                      name_split[0] + "-" + data_split_name + "-ch-" + str(channel_id) + ".out"
                                      ) 
...
# -----------------------------------------------------
    ...
    def compute_metric(self, labels, scores, metric, verbose=1, n_jobs=1):
        ...
        if scores[0].ndim == 1 or scores[0].shape[-1] == 1:
            args_local = [x + (metric,) for x in list(zip(labels, scores))]
            pool = multiprocessing.Pool(n_jobs)

            results = []
            #ipdb.set_trace(context=25)
            
            for result in pool.istarmap(self.compute_single_sample, args_local):
            ...
            for i, x, y in zip(range(n_files), labels, scores):
                results.append(self.compute_single_sample(x, y, metric))
    ...
    def compute_single_metric(self, score, label, metric):
        ...
        #import ipdb
        #ipdb.set_trace(context=25)
        #print("Whiting the single sample metric computation!")
