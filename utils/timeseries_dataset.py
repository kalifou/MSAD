...
def create_splits(data_path, split_per=0.7, seed=None, read_from_file=None):
    ...
    if read_from_file is not None:
        ...
        if 'train_set' in subsets and 'val_set' in subsets:
            train_set = [x for x in df.loc['train_set'].tolist(
            ) if not isinstance(x, float) or not math.isnan(x)]
            val_set = [x for x in df.loc['val_set'].tolist(
            ) if not isinstance(x, float) or not math.isnan(x)]

            return train_set, val_set, test_set
        elif 'train_set' in subsets and 'test_set' in subsets:
            train_val_set = [x for x in df.loc['train_set'].tolist(
            ) if not isinstance(x, float) or not math.isnan(x)]
            test_set = [x for x in df.loc['test_set'].tolist(
            ) if not isinstance(x, float) or not math.isnan(x)]
        ...
        datasets = [x for x in os.listdir(
    ...
    for dataset in datasets:
        ...
        val_idx = np.asarray(
            [x for x in range(n_timeseries) if x not in train_idx])
        # ipdb.set_trace(context=25)
...
def read_files(data_path):
    ...
    fnames = [x for x in os.listdir(
        data_path) if ".csv" in x and "tsfresh" not in x.lower()]
    ...
    if len(fnames) > 0:
        ...
        datasets = [x for x in os.listdir(
            data_path) if os.path.isdir(os.path.join(data_path, x))]
        for dataset in datasets:
...
class TimeseriesDataset(Dataset):
    def __init__(self, data_path, fnames, verbose=True):
        ...
        for fname in tqdm(self.fnames, disable=not verbose, desc="Loading dataset"):
            data = pd.read_csv(os.path.join(
                self.data_path, fname), index_col=0)
    ...
    def get_weights_subset(self, device):
        ...
        labels = np.fromiter(
            map(self.__getlabel__, range(self.__len__())), dtype=np.int16)

        # Compute weights
        labels_exist = np.unique(labels)
        labels_not_exist = [x for x in np.arange(
            len(detector_names)) if x not in labels_exist]
        sklearn_class_weights = list(compute_class_weight(
