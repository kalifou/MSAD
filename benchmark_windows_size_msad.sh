...
for window_size in 256; do # 128 256 512 1024; do
	...
	#    --path_save $save_path  \
	#    --data $path_dataset_local/train_all_targets__TEST_MODE/ESA_ADB_$window_size/
    ...
    python3 finetune_deep_model.py --path $path_dataset_local/train_all_targets__TEST_MODE/ESA_ADB_$window_size/ \
	    --model=$model_stem \
	    ...
	    --epochs=$num_epochs --batch=8 --window-size=$window_size >> logs_finetuning/logs_$model_stem$"_"$window_size.out 2>&1
