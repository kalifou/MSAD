...
path_dataset_local="data/sample_labelled_esa_adb/m2/" #"data/ESA-ADB/binarized/windowed/m2/"
save_path="results/benchmark/finetuning/"
num_epochs=1
batch_size=32
mode="__TEST_MODE" #__TEST_MODE

mkdir -p $save_path


for model_stem in "inception_time" "convnet" ; do

    for window_size in 256 ; do #1028
    
        model_name=$(basename results/weights/supervised/resnet_default_$window_size/model_*)
        mkdir -p $save_path/$window_size
    	...
    	#    --path_save $save_path/$window_size  \
        ...
        python3 finetune_deep_model.py --path $path_dataset_local/train_all_targets$mode/ESA_ADB_$window_size/ \
    	    --model=$model_stem \
