...
def create_tmp_dataset(
    ...
    name_local = '{}_{}'.format(name, window_size)
    ...
    for dataset in datasets:
        Path(os.path.join(save_dir, name_local, dataset)).mkdir(parents=True, exist_ok=True)
    ...
    for ts, label, fname in tqdm(zip(ts_list, labels, fnames), total=len(ts_list), desc='Save dataset'):
        ...
        df.to_csv(os.path.join(save_dir, name_local, dataset_name, ts_name + '.csv'))
...
def split_and_compute_labels(x, metrics_data, window_size):
    ...
    :param window_size: the size of the windows that will be created ["36", "@Channel_37"] 
